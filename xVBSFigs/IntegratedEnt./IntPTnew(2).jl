...
Labels = [L"\theta = 0,{\rm \ Dimerized}",L"\theta  \approx 0.55, {\rm \ XVBS}",L"\theta  \approx 0.63, {\rm \ XVBS}"]
...
plot!()
plot!(X2,Y2,seriestype = :scatter,color = :black , marker = :xcross, markersize = 8,label = L"{\rm XVBS \ Point, \ DMRG}")
plot!(X1,Y1,  linewidth = 12,color = :black,label = L"{\rm XVBS \ Point, \ Analytic}",seriestype = :scatter, markersize = 6, legend = :bottomright)
