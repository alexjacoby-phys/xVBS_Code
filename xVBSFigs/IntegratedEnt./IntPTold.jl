...
plot(X,Y[1], xlabel = "Site After Bipartition",ylabel = L"\mathbf{S}_{vN}" ,color = GRAD[1] ,ytickfontsize = 10,xtickfontsize = 10,xguidefontsize = 15,yguidefontsize =15, fontfamily = "Times", label = Labels[1], legend = :bottom, markersize = 6, marker = :star5,linewidth = 2, legendfontsize = 10)



plot!(X,Y[2],  linewidth = 2,color = GRAD[2],label = Labels[2], marker = :hex, markersize = 6)
plot!(X,Y[3],  linewidth = 2,color = GRAD[3],label = Labels[3], marker = :utriangle, markersize = 6)

plot!()
plot!(X2,Y2,seriestype = :scatter,color = :black , marker = :xcross, markersize = 8,label = "XVBS, DMRG")
plot!(X1,Y1,  linewidth = 12,color = :black,label = "XVBS, Analytic",seriestype = :scatter, markersize = 5, legend = :bottomright)
