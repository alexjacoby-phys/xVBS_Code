...
using ..decompositions
  ...
  mutable struct algvars{W <: Number,Q <: Qnum} <: TNparams
    ...
    SvNvec::Array{Float64,1}
    Dvec::Array{Array{Float64,1},1}
  ...
  function algvars()
    algvars{Float64,U1}("",
                        ...
                        [0.], # SvNvec
                        Vector{Vector{Float64}}(undef, 1) # Dvec
                        )
  end
  ...
  function optmps(dualpsi::MPS,psi::MPS,mpo::MPO,beta::Array{P,1},prevpsi::MPS...;params::TNparams=algvars(),
    ...
    # define onsite variables for saving entropy and density matrix
    params.SvNvec = zeros(size(psi,1))
    params.Dvec = Vector{Vector{Float64}}(undef, size(psi,1))
