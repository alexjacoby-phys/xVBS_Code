...
using ..optimizeMPS
  ...
  function SvNcheck!(i::Integer,j::Integer,D::W,truncerr::Float64,
    ...
    if params.allSvNbond
      SvNvec = -sum(h->Dmat[h]^2*log(Dmat[h]^2),sizeDsq:-1:1)
      #SvNvec = [Dmat[h]^2*log(Dmat[h]^2) for h in sizeDsq:-1:1]
  ...
  function step3S(n::Integer,j::Integer,i::Integer,iL::Integer,iR::Integer,dualpsi::MPS,psi::MPS,mpo::MPO,
    ...
    if (n == totsweeps)
        Dmat = [searchindex(D,i,i) for i = 1:size(D,1)]
        sizeDsq = length(Dmat)
        params.SvNvec[i] = -sum(h->Dmat[h]^2*log(Dmat[h]^2),sizeDsq:-1:1)
        params.Dvec[i] = Dmat
    end
